...
function findStaticDir() {
  ...
  for (const name of candidates) {
    ...
    if (fs.existsSync(p) && fs.existsSync(path.join(p, 'index.html'))) {
      console.log('Статика: ' + name); return p;
    }
  }
  try {
    const dirs = fs.readdirSync(__dirname).filter(f => {
      try { return fs.statSync(path.join(__dirname,f)).isDirectory() && f!=='node_modules' && !f.startsWith('.'); }
      catch(e){ return false; }
    });
    for (const d of dirs) {
      if (fs.existsSync(path.join(__dirname,d,'index.html'))) {
        console.log('Статика найдена: '+d); return path.join(__dirname,d);
      }
    }
  } catch(e){}
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, {headers:{'Authorization':WB_API_KEY}}, res => {
      ...
      res.on('end', ()=>{
        ...
        try{resolve(JSON.parse(body));}catch(e){reject(new Error('Ошибка парсинга'));}
      });
    });
    req.on('error',reject);
    req.setTimeout(60000,()=>{req.destroy();reject(new Error('Таймаут'));});
  });
}
...
function processData(salesRaw,ordersRaw,stocksRaw,withdraw){
  ...
  (salesRaw||[]).filter(r=>(r.saleID||'').startsWith('S')).forEach(r=>{
    ...
    salesMap[k].sold+=(r.quantity||0); salesMap[k].rev+=(r.finishedPrice||0);
  });
  (ordersRaw||[]).forEach(r=>{
    const k=r.supplierArticle+'|'+r.techSize;
  ...
  (stocksRaw||[]).forEach(r=>{
    ...
    stMap[k].wb+=(r.quantityFull||0); stMap[k].to+=(r.inWayToClient||0); stMap[k].frm+=(r.inWayFromClient||0);
  });
  ...
  Object.values(salesMap).forEach(s=>{
    ...
    rows.push({sku:s.sku,size:s.size,cat:s.cat,sold:s.sold,rev:Math.round(s.rev),
      conv:Math.round(conv*100)/100,spd:Math.round(spd*1000)/1000,
      wb:st.wb,to:st.to,frm:st.frm,eff,days:Math.round(days*10)/10,
      need,batch,fabric:FABRIC_MAP[s.cat]||'Уточнить',wd:wdSet.has(s.sku)});
  });
  ...
  rows.filter(r=>!r.wd).forEach(r=>{
    ...
    c.pos++;c.sold+=r.sold;c.rev+=r.rev;c.wb+=r.wb;c.eff+=r.eff;c.spd+=r.spd;c.need+=r.need;c.conv+=r.conv;
  });
  const cats=Object.values(catMap).map(c=>({...c,
  ...
  return {rows,cats,withdraw,summary:{
...
async function fetchData(){
  ...
  if(cache.loading) throw new Error('Уже загружается');
  cache.loading=true;
  try{
    ...
    console.log('Продаж: '+salesRaw.length); await sleep(700);
    console.log('Загружаю заказы...');
    const ordersRaw=await wbGet('/supplier/orders',{dateFrom,dateTo,flag:1});
    console.log('Заказов: '+ordersRaw.length); await sleep(700);
    ...
    cache.data=result; cache.updated=new Date().toISOString();
    console.log('Готово!'); return result;
  }finally{cache.loading=false;}
}

const WITHDRAW_FILE=path.join(__dirname,'.data','withdraw.json');
function loadWithdraw(){
  try{if(fs.existsSync(WITHDRAW_FILE))return JSON.parse(fs.readFileSync(WITHDRAW_FILE,'utf8'));}catch(e){}
...
// Данные — если пустые, загружает сам и ждёт
app.get('/api/data', async (req, res) => {
  if(!cache.data && !cache.loading){
    try{ await fetchData(); }catch(e){ console.error(e.message); }
  }
  if(!cache.data) return res.json({status:'loading',message:'Загрузка...'});
...
app.post('/api/withdraw', (req, res) => {
  ...
  saveWithdraw(list);
  if(cache.data){const s=new Set(list);cache.data.rows.forEach(r=>r.wd=s.has(r.sku));cache.data.withdraw=list;}
...
app.listen(PORT, ()=>{
  ...
  if(WB_API_KEY){
    ...
    fetchData().catch(e=>console.error('Ошибка старта:',e.message));
  }
});
